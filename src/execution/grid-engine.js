...
export function buildGridLevels({
  ...
  for (let distance = 1; distance <= legsPerSide; distance += 1) {
    legs.push({
      offset: -distance,
      price: roundToTick(
        referencePrice - distance * gridSpacing,
        tickSize,
      ),
      side: OrderSide.BUY,
...
export function evaluateKillSwitch(gridState, marketContext, config) {
  const priceDeviation = Math.abs(
    marketContext.lastPrice - gridState.referencePrice,
  );

  const maxDeviation =
    config.maxDeviationSpacings * gridState.gridSpacing;

  if (priceDeviation > maxDeviation) {
    return {
      triggered: true,
      reason: "price moved beyond max grid deviation",
    };
  }

  const totalPnl =
    marketContext.realizedPnl + marketContext.unrealizedPnl;
...
function sideExposure(legs, side) {
  return legs
    .filter(
      (leg) =>
        leg.side === side &&
        [GridLegStatus.OPEN, GridLegStatus.FILLED].includes(
          leg.status,
        ),
    )
...
export function decidePendingEntries(gridState, config) {
  let buyExposure = sideExposure(
    gridState.legs,
    OrderSide.BUY,
  );

  let sellExposure = sideExposure(
    gridState.legs,
    OrderSide.SELL,
  );
  ...
  for (const leg of gridState.legs) {
    ...
    if (leg.side === OrderSide.BUY) {
      if (
        ...
        config.maxPositionQuantityPerSide
      ) {
        continue;
      }
...
export function extendGridIfNeeded(gridState, config) {
  ...
  const buyLegs = gridState.legs.filter(
    (leg) => leg.side === OrderSide.BUY,
  );

  const allBuyLegsFilled =
    buyLegs.length > 0 &&
    buyLegs.every(
      (leg) => leg.status === GridLegStatus.FILLED,
    );

  if (
    allBuyLegsFilled &&
    buyLegs.length < config.maxLegsPerSide
  ) {
    const nextOffset =
      Math.min(...buyLegs.map((leg) => leg.offset)) - 1;

    newLegs.push({
      offset: nextOffset,
      price: roundToTick(
        gridState.referencePrice +
          nextOffset * gridState.gridSpacing,
        config.tickSize,
      ),
      side: OrderSide.BUY,
      status: GridLegStatus.PENDING,
  ...
  const sellLegs = gridState.legs.filter(
    (leg) => leg.side === OrderSide.SELL,
  );
  ...
  if (
    allSellLegsFilled &&
    sellLegs.length < config.maxLegsPerSide
  ) {
    const nextOffset =
      Math.max(...sellLegs.map((leg) => leg.offset)) + 1;
...
export function computeExitOrder(
  filledLeg,
  gridState,
  config,
) {
  const exitDirection =
    filledLeg.side === OrderSide.BUY ? 1 : -1;

  const exitPrice = roundToTick(
    filledLeg.price +
      exitDirection * gridState.gridSpacing,
    config.tickSize,
  );

  return {
    side:
      filledLeg.side === OrderSide.BUY
        ? OrderSide.SELL
        : OrderSide.BUY,
    price: exitPrice,
...
export class GridEngine {
  constructor({
    ...
    this.processedEntryKeys = new Set();
    this.processedExitKeys = new Set();
  }

  start(referencePrice, atr) {
    const gridSpacing = roundToTick(
      atr * this.config.spacingMultiplier,
      this.tickSize,
    );
    ...
    this.processedEntryKeys.clear();
    this.processedExitKeys.clear();
  }
  ...
  async onTick(marketContext) {
    ...
    if (killSwitch.triggered) {
      return this.triggerKillSwitch(
        killSwitch.reason,
      );
    }

    for (const leg of extendGridIfNeeded(
      this.state,
      {
        ...this.config,
        tickSize: this.tickSize,
    ...
    const entries = decidePendingEntries(
      this.state,
      this.config,
    );
  ...
  async onEntryFilled(idempotencyKey) {
    if (this.processedEntryKeys.has(idempotencyKey)) {
      return;
    }

    const leg =
      this.legsByEntryKey.get(idempotencyKey);

    if (!leg) {
      throw new Error(
        `no grid leg found for entry idempotency key: ${idempotencyKey}`,
      );
    }

    this.processedEntryKeys.add(idempotencyKey);

    assertValidGridLegTransition(
      leg.status,
      GridLegStatus.FILLED,
    );
  ...
  async onExitFilled(
    ...
    if (this.processedExitKeys.has(idempotencyKey)) {
      return;
    }

    const leg =
      this.legsByExitKey.get(idempotencyKey);

    if (!leg) {
      return;
    }

    this.processedExitKeys.add(idempotencyKey);

    assertValidGridLegTransition(
      leg.status,
      GridLegStatus.CLOSED,
    );

    leg.status = GridLegStatus.CLOSED;

    this.legsByExitKey.delete(
      idempotencyKey,
    );
  ...
  async triggerKillSwitch(reason) {
    ...
    for (const leg of this.state.legs) {
      if (leg.status === GridLegStatus.OPEN) {
        const entryKey = buildIdempotencyKey(
          this.tradingSymbol,
          leg.offset,
          leg.cycle,
          "entry",
        ...
        await this.orderStateManager.cancelOrder(
          entryKey,
        );
      }

      if (leg.status === GridLegStatus.FILLED) {
        const exitKey = buildIdempotencyKey(
          this.tradingSymbol,
          leg.offset,
          leg.cycle,
          "exit",
        ...
        const flattenKey = buildIdempotencyKey(
          this.tradingSymbol,
          leg.offset,
          leg.cycle,
          "flatten",
